...
if [ "$SHOW_ONLY" = true ]; then
    ...
    for f in .env "$HOME/.hummingbot/.env" "$HOME/.env"; do
        if [ -f "$f" ]; then
            echo ""
            echo "Found: $f"
            grep -E "^HUMMINGBOT_API|^API_URL|^API_USER|^API_PASS" "$f" 2>/dev/null | sed 's/API_PASS=.*/API_PASS=***/' || echo "  (no API config found)"
        fi
    done
    ...
    echo "  API_URL: ${HUMMINGBOT_API_URL:-${API_URL:-http://localhost:8000}}"
    echo "  API_USER: ${HUMMINGBOT_API_USER:-${API_USER:-admin}}"
    echo "  API_PASS: ***"
    exit 0
fi
...
# Generated by hummingbot-api-setup skill

HUMMINGBOT_API_URL=$API_URL
HUMMINGBOT_API_USER=$API_USER
HUMMINGBOT_API_PASS=$API_PASS
EOF
